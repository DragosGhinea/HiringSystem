...
const InterviewerRow = ({user, index}) => {
    ...
    const onDeleteClick = () => {
        if(deleteRef.current.innerText === "Are you sure?"){
            jwtInterceptor.post(`http://localhost:8081/api/v1/interviewer/delete/${user.id}`)
                .then(data => {
                    setDeleted(true);
                })
        }
    ...
    return (
        <tr>
            ...
            <td className="align-middle">{user.interviewerType}</td>
